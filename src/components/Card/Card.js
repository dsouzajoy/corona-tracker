...
function Card({ title, value, delta }) {
    return (
        <div className={title === "Total Cases" ? "card card-blue"
        : title === "Active Cases" ? "card card-red" 
        : title === "Recovered" ? "card card-green" 
        : title === "Deceased" && "card card-grey"} >
            {
                title === "Total Cases" ? <i className="fas fa-virus fa-4x"></i> 
                : title === "Active Cases" ? <i className="fas fa-viruses fa-4x"></i> 
